...
import {
  cantidad_de_registros_en_un_mes,
  Registro,
  RegistroBusqueda,
...
export class RegistroRepository {
  public async getRegistrosBusquedaDocente(
    registroDTO: RegistroBusquedaDocente,
  ): Promise<RegistroResponse[]> {
    try {
      const [rows]: any = await pool.query(
        "CALL sp_buscar_tabla_docente(?,?,?,?,?)",
        [
  ...
  public async getRegistrosBusqueda(
    registroDTO: RegistroBusqueda,
  ): Promise<RegistroResponse[]> {
    try {
      const [rows]: any = await pool.query(
        "CALL sp_buscar_tabla_admin(?,?,?,?,?)",
        [
          registroDTO.anio,
          registroDTO.mes,
  ...
  public async getCantidad_de_registros_en_un_mes(
    idusuario: number,
  ): Promise<cantidad_de_registros_en_un_mes[]> {
    const [rows]: any = await pool.query(
      "CALL sp_tabla_recuperar_cantidad_de_registros_en_un_mes(?)",
      [idusuario],
    );
    return rows[0] as cantidad_de_registros_en_un_mes[];
  }

  public async insert_tabla_registro(
    tablaRegistro: TablaRegistro,
  ): Promise<boolean> {
    try {
      const alumnosJSON = JSON.stringify(tablaRegistro.alumnos);
      await pool.query(
        "CALL sp_tabla_registro_insert(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
  ...
  public async insertTablaMultiple(jsonData: any[]): Promise<boolean> {
    try {
      await pool.query("CALL sp_tabla_insert_multiple(?)", [
